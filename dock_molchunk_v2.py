...
# molchunk_path = '/data/dopamine_3_results/mol_chunk_docking/mol_chunks_test_5001_8000.molchunk'
path = pathlib.Path(molchunk_path)
print(path)
if path.is_file():
    file_prefix = path.stem

    print(f'Loading {path} for docking. Found molchunk file {file_prefix}') 
    ...
    docking_dir = pathlib.Path(working_dir_path / file_prefix)
    docking_dir.mkdir()

    print(f'Creating directory for docking output: {docking_dir}')

    # working_dir = '/data/dopamine_3_results/mol_chunk_docking/test_5001_8000'
    ...
    df = remove_duplicates(df)
    # mols_to_pymol(df)
    outdf = run_autodock_gpu(df, col_to_dock, autodock_gpu, lsmet, num_runs, docking_dir, receptor_path, dev_num)
    indexed_df = index_docking_output(outdf)
    final_df = extract_specific_pdbqt(indexed_df, num_runs, docking_dir)
    # show_docked(final_df)
    # del final_df['ROMol']
    outfile = pathlib.Path(path.parent / (file_prefix + '_out.molchunk'))
    feather.write_feather(final_df, outfile)
else:
    print(f'Could not locate {path}, please check the filepath.')
