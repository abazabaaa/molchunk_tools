...
def mol_chunk_from_smi(smileslist, nameslist, prefix, output_dir):
    ...
    for count,smile in enumerate(tqdm(smileslist, total=len(smileslist), smoothing=0)):
        try:
...
#             print(type(mol))
    ...
    #         print(type(mol3))
            ...
            smiles = str(smileslist[count])
            name = nameslist[count]
            console.print(f'Failed with oechem, falling back to corina, converting {name} with smiles {smiles} to PDBQT', style="bold red")
            
#             print(type(mol))
            try:
                mol = get_3d_from_corina(smiles, name, output_dir, count)
                # mol2 = generate_conformers(mol, max_confs=800, strictStereo=True, ewindow=15.0, rms_threshold=1.0, strictTypes = True)
        #             print(type(mol2))
                mol3 = get_charges(mol, max_confs=800, strictStereo=True,
                normalize=True, keep_confs=None, legacy=False)
        #         print(type(mol3))
                ...
                console.print(f'{name} completed', style="bold blue")
    ...
    mol_chunk = pd.DataFrame(list(zip(names_list, smiles_list, mol2_block_am1bcc_list, pdb_am1bcc_list, pdbqt_am1bcc_list, pdbqt_gast_list)), \
                             columns = ['names', 'smiles', 'mol_block_am1bcc', 'pdb_block_am1bcc', 'pdbqt_block_am1bcc', 'pdbqt_gast_list'])
...
#             duplicateRowsDF2 = df[df.duplicated(['names'])]
#             if duplicateRowsDF2.shape[0] != 0:
#                 indices = duplicateRowsDF2.index
#                 names = duplicateRowsDF['names']
#                 for count,indexid in enumerate(indices):
#                     print(df.loc[indexid,'names'])
#                     new_value = f'{names[indexid]}_{str{dupenum}}'
#                     print(new_value)
#                     df.at[indexid,'names'] = new_value
#                     duplicateRowsDF3 = df[df.duplicated(['names'])]
#                     if duplicateRowsDF3.shape[0] != 0:
#                         print('fix duplicate name entries in smiles')
#                         return None
#                     else:
#                         print('duplicates_resolved')
...
dude_smi_file = '/data/dopamine_3_results/d3_glide_dockop/d3_chembl_fromligprep.smi'
delimiter = ' '
df = df_from_dude_smi_file(dude_smi_file, delimiter, 1, 0)
smileslist = list((df['smiles'])[4000:6000])
nameslist = list((df['names'])[4000:6000])
print(len(smileslist))
print(len(nameslist))


prefix = 'd3_chembl_fromligprep_first4000_6000'
output_dir = '/data/dopamine_3_results/d3_glide_dockop/d3_chembl_first2k'
